...
export default class UserController{
    ...
    public async deleteRule(request: Request, response: Response): Promise<Response> {
        try {
           ...
           if(result){
            ...
            return response.status(200).json("Rule remove successfully " + rule);
           }

           logger.warn("/user. AddRule method response successfully", rule);
           return response.status(400).json("Rule remove unsuccessfully " + rule);
        } catch (error) {
            logger.warn("/user. AddRule method response successfully", error);
            return response.status(400).json("Rule remove unsuccessfully " + error);
        }
    }
    ...
    public async login(request: Request, response: Response): Promise<Response>{
        ...
        try{
            ...
            logger.info("/user/login . login method response unsucessfuly", {email});
            return response.status(401).json("Dadis incorretos");
